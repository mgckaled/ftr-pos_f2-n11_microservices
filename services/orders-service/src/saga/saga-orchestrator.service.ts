...
export class SagaOrchestratorService {
		...
		const steps: SagaStep[] = [
			...
			},
			{
				...
				action: async (context) => {
					...
					await this.ordersRepository.update({ orderId: context.orderId }, { status: 'CONFIRMED' })
				...
				compensation: async (context) => {
					...
					await this.ordersRepository.update({ orderId: context.orderId }, { status: 'CANCELLED' })
		...
		try {
			...
			for (let i = 0; i < steps.length; i++) {
				...
				try {
					...
					await this.recordStepExecution(sagaInstance.sagaId, step.name, 'FAILED', error.message)
	...
	private async compensate(sagaId: string, steps: SagaStep[], context: any): Promise<void> {
		...
		for (const execution of executedSteps) {
			...
			if (step) {
				try {
					...
					await this.sagaStepRepository.update({ id: execution.id }, { status: 'COMPENSATED' })
	...
	private async recordStepExecution(
		...
		if (existing) {
			await this.sagaStepRepository.update({ id: existing.id }, { status, errorMessage })
		} else {
